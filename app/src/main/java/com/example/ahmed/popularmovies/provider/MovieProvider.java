...
public class MovieProvider extends ContentProvider {
    ...
    private static final int MOVIE_WITH_REVIEW = 102;
    // .../movie/movie_id/trailer
    private static final int MOVIE_WITH_TRAILER = 103;
        ...
        switch (match) {
            ...
            case MOVIE_WITH_REVIEW: {
                long _id = db.insertWithOnConflict(MovieContract.ReviewEntry.TABLE_NAME, null,
                                                   values, SQLiteDatabase.CONFLICT_REPLACE);
                if (_id > 0) {
                    returnUri = MovieContract.MovieEntry.buildMovieIdWithReview(movie_id);
                } else {
                    throw new android.database.SQLException("Failed to insert row into " + uri);
                }
                break;
            ...
            case MOVIE_WITH_TRAILER: {
                long _id = db.insertWithOnConflict(MovieContract.TrailerEntry.TABLE_NAME, null,
                                                   values, SQLiteDatabase.CONFLICT_REPLACE);
                if (_id > 0) {
                    returnUri = MovieContract.MovieEntry.buildMovieIdWithTrailer(movie_id);
                } else {
                    throw new android.database.SQLException("Failed to insert row into " + uri);
                }
                break;
